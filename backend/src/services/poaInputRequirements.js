...
exports.derivePoaInputRequirements = void 0;
const INPUT_REQUIREMENTS_SCHEMA_VERSION = "2026-03-27";
const buildEqualsCondition = (fact, value) => ({
    all: [
        {
            fact,
            operator: "equals",
...
const hasExecutionPath = (executionPaths, key) => {
    return executionPaths.some((path) => path.key === key);
};
const buildExecutionPaths = (record) => {
    if (record.ui_profile === "review_required" ||
    ...
    if (record.ui_profile === "notary_and_witness" ||
    ...
    if (record.ui_profile === "notary_or_witness" ||
    ...
    if (record.ui_profile === "witness_only" ||
...
const buildPrincipalSection = (context) => {
    const fields = [
        {
...
const buildAgentSection = (context) => ({
    key: "agent",
    title: "Agent",
    presence: hasExecutionPath(context.executionPaths, "manual_review")
        ? "manual_review"
...
const buildAuthorityScopeSection = (context) => {
    const fields = [];
    if (context.record.special_authority_rule === "required_for_certain_acts") {
        fields.push({
            ...
            collectFrom: "member",
            defaultSource: "none",
            helpText: "Collect explicit authority selections for powers that require additional authorization.",
        });
    }
...
const buildDurabilitySection = (context) => {
    const fields = [];
    let presence = "hidden";
...
const buildEffectiveDateSection = (context) => {
    const fields = [];
    let presence = "hidden";
...
const buildWitnessSection = (context) => {
    let presence = "hidden";
    let appliesToPaths = [];
    ...
    if (presence !== "hidden") {
        fields.push({
            ...
            dataType: "integer",
            collectFrom: "member",
            defaultSource: typeof context.record.witness_count === "number" ? "jurisdiction_default" : "none",
            validation: typeof context.record.witness_count === "number"
                ? {
            ...
            ...(typeof context.record.witness_count === "number"
                ? {}
                : {
                    helpText: "Witness count is not normalized for this jurisdiction yet and needs confirmation.",
                }),
        });
    }
    return {
...
const buildNotarySection = (context) => {
    let presence = "hidden";
    let appliesToPaths = [];
...
const buildStatutoryNoticesSection = (context) => {
    const fields = [];
...
const buildDocumentOutputs = (context) => {
    const outputs = [
        {
...
const buildNotices = (context) => {
    ...
    if ((context.record.witness_rule === "additional_to_notary" ||
...
const applyJurisdictionOverrides = (requirements, record) => {
    if (record.jurisdiction === "US-FL") {
        ...
        const notices = requirements.notices.filter((notice) => notice.key !== "witness_count_needs_confirmation");
        return {
            ...requirements,
...
const derivePoaInputRequirements = (record) => {
    const executionPaths = buildExecutionPaths(record);
    const initialDerivationMode = record.ui_profile === "review_required" || record.review_status === "needs_review"
        ? "manual_review"
        : "rules_only";
    const context = {
        record,
    ...
    if (hasExecutionPath(executionPaths, "witness_execution") ||
...
exports.derivePoaInputRequirements = derivePoaInputRequirements;
//# sourceMappingURL=poaInputRequirements.js.map