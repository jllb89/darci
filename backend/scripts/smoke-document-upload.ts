...
const createTextBuffer = (byteLength: number) => {
...
const finalizeUpload = async (documentId: string, documentVersionId: string) => {
...
const runHappyPath = async () => {
  ...
  const created = payload as DocumentResponse;
  ...
  assertStatus(
    ...
    200,
    "Finalize should return 200"
  );
  const finalized = finalizeResult.payload as DocumentResponse;
  assertTruthy(finalized.document.idn, "Finalize should assign IDN");
  assert(
    finalized.document.status === "pending_signature",
    "Status should be pending_signature"
  );
};
...
const runNonPdfFinalizeRejection = async () => {
  ...
  assertStatus(
    ...
    finalizeResult.payload,
    400,
    "Non-PDF finalize should return 400"
  );
};
...
const runOversizeFinalizeRejection = async () => {
  ...
  assert(
    response.status === 201,
    "Oversize finalize create should return 201"
  );
  const created = payload as DocumentResponse;
  ...
  assertStatus(
    ...
    finalizeResult.payload,
    400,
    "Oversize finalize should return 400"
  );
};
