...
process.stdin.on('end',async()=>{
  try{
    ...
    for(const entry of entries){
      ...
      await fs.stat(file);
      await xattr.set(file,'user.supabase.content-type',entry.metadata?.mimetype??'application/octet-stream');
      await xattr.set(file,'user.supabase.cache-control',entry.metadata?.cacheControl??'no-store');
    }
