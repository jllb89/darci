...
if (!documentId) {
  console.error(
    "Usage: ts-node scripts/verify-audit-events.ts --document-id <uuid> [--signature-id <uuid>]"
  );
  process.exit(1);
}

const fetchByDocumentId = async (docId: string) => {
...
const fetchBySignatureId = async (sigId: string) => {
  const { data, error } = await supabase
    .from("audit_events")
    .select("action, entity_type, entity_id, metadata, created_at")
...
const verifyActions = (actions: string[], required: string[]) => {
...
const main = async () => {
  ...
  const missingDocument = verifyActions(
    documentActions,
    requiredDocumentActions
  );
  ...
  console.log(
    ...
    missingDocument.length ? missingDocument.join(", ") : "none"
  );
