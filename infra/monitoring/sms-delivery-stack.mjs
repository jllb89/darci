...
export function buildSmsDeliveryTemplate() {
  const account='427057633951', region='us-east-1', name='darci-staging-auth-sms';
  const topic=`arn:aws:sns:${region}:${account}:${name}-events`;
  const log=`arn:aws:logs:${region}:${account}:log-group:/darci/staging/auth-sms-delivery`;
  return {AWSTemplateFormatVersion:'2010-09-09', Description:'Staging SMS receipts; sanitized logs only; no SMS sender or customer messages.', Resources:{
    Logs:{Type:'AWS::Logs::LogGroup',DeletionPolicy:'Retain',UpdateReplacePolicy:'Retain',Properties:{LogGroupName:'/darci/staging/auth-sms-delivery',RetentionInDays:30}},
    ...
    Handler:{Type:'AWS::Lambda::Function',DependsOn:'Logs',Properties:{FunctionName:name+'-receipts',Runtime:'nodejs22.x',Handler:'index.handler',Timeout:15,MemorySize:128,
      Role:{'Fn::GetAtt':['Role','Arn']},LoggingConfig:{LogGroup:{Ref:'Logs'}},Environment:{Variables:{EXPECTED_TOPIC_ARN:{Ref:'Topic'}}},Code:{ZipFile:readFileSync(new URL('./sms-delivery-handler.cjs',import.meta.url),'utf8')}}},
    ...
    HandlerAlarm:{Type:'AWS::CloudWatch::Alarm',Properties:{AlarmName:'darci-recovery-staging-sms-receipt-handler',Namespace:'AWS/Lambda',MetricName:'Errors',Dimensions:[{Name:'FunctionName',Value:{Ref:'Handler'}}],Statistic:'Sum',Period:300,EvaluationPeriods:1,Threshold:1,ComparisonOperator:'GreaterThanOrEqualToThreshold',TreatMissingData:'notBreaching',AlarmActions:[`arn:aws:sns:${region}:${account}:darci-recovery-critical`]}},
    DeliveryAlarm:{Type:'AWS::CloudWatch::Alarm',Properties:{AlarmName:'darci-recovery-staging-sms-receipt-routing',Namespace:'AWS/SNS',MetricName:'NumberOfNotificationsFailed',Dimensions:[{Name:'TopicName',Value:name+'-events'}],Statistic:'Sum',Period:300,EvaluationPeriods:1,Threshold:1,ComparisonOperator:'GreaterThanOrEqualToThreshold',TreatMissingData:'notBreaching',AlarmActions:[`arn:aws:sns:${region}:${account}:darci-recovery-critical`]}},
