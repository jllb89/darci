...
**Production addition, 23 September:** `darci-production-edge-audit` now installs ten native ECS/ALB alarms named `darci-recovery-production-capacity-*`, matching the existing SNS source policy. They cover API/web/worker CPU ≥85% or memory ≥80%, API/web healthy targets <2, API target 5xx ≥5/minute and API p95 latency ≥3s (3 of 5 one-minute periods). Low-sample p95 evaluation is ignored; low-traffic `INSUFFICIENT_DATA` does not prove latency health. Capacity alarms intentionally send ALARM only, not initial OK/insufficient-data emails. Verify recovery using metrics/readiness and explicitly close the incident. Existing application/backup alarms retain their own recovery behavior. Source-failure acceptance of the ten new alarms is still required. [Current controls/evidence](production-provider-hardening-2026-09-23.md).
