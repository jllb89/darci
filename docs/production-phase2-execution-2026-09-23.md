...
**Later hardening/provider pass:** production now has **105 migrations** after the rehearsed backend-only-table access fix (staging explicitly unchanged). Direct DB networking is restricted, WAF/CloudTrail and ten capacity alarms are deployed, the post-hardening backup/3-PDF recovery passed, and production Supabase SMTP is configured with login/signup still closed. Live Stripe prices/Portal are mapped but sales remain disabled. Server Maps fallback is deployed and verified through both NATs; API7/worker6/web4 are healthy at 2/1/2. See [the current evidence and remaining gates](production-provider-hardening-2026-09-23.md), which supersedes historical counts/pending infrastructure statements below. No new application images or public application access were deployed in this hardening pass.

**Production email follow-through — 18:17 UTC:** the approved Resend-only configuration release reached `UPDATE_COMPLETE`, preserving accepted release `442baee` image digests. The exact signed callback is public; app/API routes remain operator-only. The deployed-config ECS test passed actual delivery, callback/replay/privacy checks, and Jorge confirmed receipt. API6/worker5/web4 are healthy at 2/1/2; ten alarms are OK. Temporary deployment permissions were removed. General notification runners/signup/payments stay closed. See [email acceptance](production-email-acceptance-2026-09-23.md); this supersedes earlier pending email setup statements in this record.

**Current acceptance follow-through:** actual approved GitHub release **35894433475** passed at `442baee`; the real API-only 503/circuit-breaker/CloudFormation automatic rollback also passed at **17:54 UTC** with exact configuration/image recovery. All 12 sampled readiness requests returned 200 and two original API tasks remained healthy. The approved current operator `/32` has replaced the old network entry, and temporary network permissions were removed. [Latest evidence](production-release-verification-2026-09-23.md). The older pending-OIDC/rollback notes below are historical. Broader Phase 2 and provider/human gates remain open.
