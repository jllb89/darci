...
- [ ] Protect/backfill legacy identity copies and accept retention/legal-hold behavior across GPS, caches, metadata and logs. No automatic deletion until approved.
- [ ] Accept distributed abuse behavior and browser/CSP/production-origin posture; complete privileged enrollment/operator UI and non-billing step-up controls.
...
- [ ] Deploy/retest the completed-package retry fix through the API. Do not confuse the local read-only service proof with deployed acceptance.
...
- [ ] Exercise delayed/out-of-order events, last-unit concurrency, worker crash/retry, upgrades without usage reset and recovery from a deliberately missed event. Isolated SQL allowance/reversal/hold/lease tests already pass; deployed drills remain open.
...
- [x] Actual deployed operator resync, duplicate held-release retry and controlled reversal pass with recent TOTP. Replay/missed-event and broader outage drills remain part of the open item above.
...
- [ ] Deploy/recheck the provider-deferral fix and reconcile the existing synthetic notification jobs. No live notification state was rewritten or manually replayed; already accepted provider messages may continue their own retries.
- [ ] Deploy source emitters/watchdog, verify genuine heartbeat metrics, enable missing-heartbeat actions and prove actual originating failure/recovery paths. Installed category filters alone do not close this gate; watchdog currently has actions disabled intentionally.
