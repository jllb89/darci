...
- The original audit changed documentation only. **22 September latest: staging deployment `eeec032` passed; the downgrade fix now passes through the deployed API. Phase 1 acceptance remains incomplete; production is not approved.** Current evidence is in [post-deployment acceptance](production-readiness-acceptance-2026-09-22.md) and [the execution record](production-hardening-phase1-execution-2026-09-17.md). Findings below describe the original audit baseline unless superseded by those records. The Phase 1 checklist now separates completed implementation/component proofs from outstanding end-to-end acceptance.
...
- **22 September billing acceptance: 13/15 evidence categories**, up from 9/15. Failed payment, authentication-required, cancellation and downgrade pass deployed checks; the earlier real test-clock rollover also passed. Final-package hold and controlled usage reversal remain missing. The approved clock-aware reconciliation fix passes **626 backend tests** and a read-only staging scan (**17/17 provider subscriptions, zero issues/backlog**), but is **local, not deployed**. Actual staging remains `test + enforced`; the report recommends `remain_observe`. This pass changed neither mode.
...
- [ ] Assign remaining legal/privacy/commercial owners and acceptance reviewer; confirm production domains, concurrency and App Store scope. Obtain retention/legal-hold and commercial approvals. Do not confuse resolved engineering choices with legal approval.
...
- [x] Record **13/15 actual lifecycle evidence categories**, without inventing an acceptance ID. Existing Checkout-completion, usage and upgrade/release evidence is historical; this does not prove every outcome this pass.
- [ ] Complete the two missing categories: **final-package billing hold and controlled usage reversal**. Prove same-byte hold → resubscribe → release, not merely a historical release event.
...
- [ ] Deploy the reconciliation fix and reverify the operator report. Current deployed `eeec032` still uses the old list-only report; no acceptance marker or genuine alert was suppressed.
...
- [x] Repair deployment validation: exact-SHA server CI, early schema/key preflight, parallel image builds/scans, scanned-digest rollout and last-successful-rollout change detection. iOS is independent. Latest server CI/deployment succeed at `eeec032`; current iOS optimization work is separate and not counted as completed.
