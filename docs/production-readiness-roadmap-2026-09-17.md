...
- [x] Create approved machine-only release roles and document deployments/environment configuration. Manual exact-CI/scanned-image workflow published in `4f29c95`; an actual GitHub OIDC run remains unchecked.
...
- [ ] Publish the verification changes and apply the scoped production ECS read permissions before exercising the protected workflow.
- [ ] Exercise actual GitHub OIDC promotion and application-health-triggered failed-release rollback. Configuration-error rollback was already observed; it is not a substitute for the application-health case.
