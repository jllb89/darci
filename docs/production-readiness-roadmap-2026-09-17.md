...
- [ ] Complete abrupt mid-effect Stripe crash acceptance. The previously proven expired-lease/provider-authoritative recovery and the new abrupt PDF-generation exit do not substitute for every payment interruption point.
...
- [x] Resolve OTLP configuration: Jorge explicitly approved disabling it. Both staging deployment task definitions set `OTEL_SDK_DISABLED=1`; AWS log/metric/alert routing stays enabled, Sentry remains deferred. Running-task verification is recorded with this batch's deployment evidence.
- [ ] Complete outbox/OTP/bounce/suppression delivery proof. Version the remaining production-ready monitoring configuration and operator procedures.
