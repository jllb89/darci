...
- [ ] Complete remaining full hosted actor/Storage/Realtime/auth policy acceptance; table RLS coverage is not proof of every policy.
...
- [ ] Confirm consecutive automatic production backup runs, RPO cadence and representative-volume recovery performance; synthetic on-demand recovery does not close those operational gates.
...
- [x] Prepare ignored production credential fields, preserve supplied values, reuse the approved distinct staging Maps keys, and keep secrets out of logs/Git. Missing APNs credentials and engineering-generated SMS/Stripe webhook secrets are explicit.
...
- [ ] Confirm Stripe account requirements in the owner Dashboard (not exposed by this account API response), deploy production callback/runtime configuration, and complete the separately approved activation/payment acceptance sequence.
...
- [ ] Complete production Stripe webhook/runtime deployment and live merchant/checkout/Portal acceptance; catalog/Portal preparation is already closed above. Get explicit authorization for any real charge.
