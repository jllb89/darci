...
- [x] Create approved machine-only release roles and document deployments/environment configuration. Manual exact-CI/scanned-image workflow prepared locally; publishing and an actual GitHub OIDC run remain unchecked.
...
- [ ] Commit/publish and exercise the approved GitHub OIDC promotion workflow; prove application-health-triggered failed-release rollback and full PDF-worker recovery. Real CloudFormation configuration-error rollback was observed and corrected; it is not a substitute for every release-failure scenario.
