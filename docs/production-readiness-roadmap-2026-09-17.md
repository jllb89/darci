...
- The original audit changed documentation only. **22 September latest: staging `c5acd9b` passed CI/deployment; the reconciliation fix passes the actual admin API. Phase 1 acceptance remains incomplete; production is not approved.** Current evidence is in [the latest Phase 1 pass](production-readiness-phase1-pass-2026-09-22.md), [earlier post-deployment acceptance](production-readiness-acceptance-2026-09-22.md) and [the execution record](production-hardening-phase1-execution-2026-09-17.md). Findings below describe the original audit baseline unless superseded by those records. Completed-package retry and independent alert source changes are local pending the next app deployment.
...
- **Automatic recovery is enabled:** 02:00/14:00 Mexico City, short-lived tasks only. Latest complete snapshot checked on 22 September contains 2,533 objects; both recovery alarms are OK. Whole-application recovery still needs acceptance. Application hardening/public verification is deployed; anonymous and authenticated public verification both expose metadata only.
- **22 September billing acceptance: 15/15 actual evidence categories**, including new real-API hold, real-TOTP-controlled reversal and payment-webhook reactivation of byte-identical final PDF evidence. The deployed admin reconciliation is clean; the independent closeout scan checks **21/21 subscriptions, zero billing issues/Stripe backlog**. This is category coverage, not full CA/OH/device/outage acceptance; explicit team sign-off remains unset. Current local backend suite: **663 passing tests**. Staging remains `test + enforced`; no mode or live-payment activation was changed. Closeout also found/fixed a provider-deferral duplicate-send loop locally; notification health needs deployed recheck.
- **22 September additional decisions:** Jorge is the assigned policy reviewer; exact legal/commercial wording remains unapproved. Additional staging monitoring spend up to $6/month approved. Seven failure-category AWS detectors are installed on the existing email route; watchdog actions wait for genuine deployed heartbeats. `master` has no protection/ruleset; Jorge explicitly chose to leave repository settings unchanged for now.
...
**Latest verified deployment: `d166019` (22 September).** Server CI and staging rollout passed. The resumed pass closed the deployed retry/MFA/refresh gates, encrypted all 44 approved legacy identity copies, verified delayed/duplicate Stripe replay, enabled genuine watchdog monitoring, and reconstructed Auth/Storage/API with exact PDF access on an isolated backup. Remaining unchecked items below are acceptance gaps, not a claim that those completed implementations are still undeployed. Production remains unopened.
...
- [ ] Complete the full deployed owner/signer/selected-and-wrong-notary/admin/anonymous/worker private-and-held-document matrix, including legacy notary codes and invite flows.
...
- [ ] Accept actual production-origin/CSP configuration and complete operator UI interaction acceptance. Local production-shaped API checks do not prove final production DNS/browser configuration.
...
- [ ] Complete storage/database/crash/partial-package recovery and large/resource-limit drills: no lost material evidence, duplicate signature, overwritten version or false completed/released/anchored state.
- [ ] Prove finalized/held/released transitions and exact readable published bytes across all authorized routes, retries and billing reactivation. Any historical PDF repair remains separately authorized.
...
- [ ] Complete isolated whole-application restore, corrupted/missing-object injection, key/Auth ownership recovery, private/held/public access and queue/job reconstruction. No replayed invitations, duplicate charges or automatic held release.
- [ ] Demonstrate full application RTO within 4h and ongoing RPO within 24h; component times alone do not close this gate. Accept retention/cost/credential-rotation policy and stronger separate-account production isolation.
...
- [ ] Complete remaining upgrade-with-nonzero-usage and abrupt mid-effect crash acceptance. Historical upgrade/lifecycle evidence and SQL idempotency tests do not substitute for every interruption point.
- [x] Deployed last-unit concurrency: four simultaneous real review approvals on a three-unit Starter fixture yielded exactly three 200s, one 409 limit rejection and three consume records. Approved-document retries consumed nothing extra; exhausted eligibility denied new work. Delayed/out-of-order/crash recovery remains open above.
...
- [x] Actual deployed operator replay/out-of-order drill: genuine TOTP admin replays subscription-deleted, then delayed subscription-created, then duplicate created. Provider-authoritative state remains canceled; no usage rows change; all three reason-bound audits record the exact operator. AAL1 is rejected; temporary role/factor/session removed. Missed-event and worker-crash recovery are still distinct open scenarios.
...
- [ ] Prove the remaining originating failure/recovery paths. Enabled filters/heartbeats and earlier synthetic SNS routing do not prove every application incident source.
- [ ] Complete critical-category detectors/queue-age thresholds and safe failure → notification → acknowledgment → recovery drills for auth, email/SMS, PDF/finalization, audit, Stripe/drift/held release and retention.
- [ ] Complete outbox/OTP/bounce/suppression delivery proof and verify OTLP or explicitly disable it. Version the remaining production-ready monitoring configuration and operator procedures.
...
- [ ] Confirm required branch-protection/release-review settings, release any subsequent local fixes and complete all remaining cross-track acceptance before marking Phase 1 done.
