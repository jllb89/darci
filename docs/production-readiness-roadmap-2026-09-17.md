...
- **22 September current status:** runtime hardening at `d166019` passed CI/deployment and real API checks; recovery/legacy-protection tooling and evidence are committed in `b18e154`. Completed-package retry, admin MFA, concurrent refresh/revocation, notification deferral, billing replay/resync/expired-lease recovery and independent watchdog monitoring are no longer merely local TODOs. **Phase 1 still has the explicitly unchecked acceptance gates in Section 7; production is not approved.** See [the latest Phase 1 evidence](production-readiness-phase1-pass-2026-09-22.md). Sections 2–6 retain the dated audit background; Section 7 is the authoritative current checklist.
...
1. **Remaining hardening acceptance:** complete held/signer/legacy-route coverage, selective queue restoration, nonzero-usage upgrades/mid-effect interruptions, production CSP/origin posture and remaining incident-source diagnostics. Environment isolation, hash-only completion, atomic invites, protected identity, material audit transactions and required server CI are implemented—not still the original audit's missing features.
