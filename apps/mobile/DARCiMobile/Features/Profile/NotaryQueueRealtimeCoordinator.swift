...
@MainActor
protocol NotaryQueueRealtimeProviding: AnyObject {
    func start(
        queueUserId: String,
        accessToken: String,
...
@MainActor
final class NotaryQueueRealtimeCoordinator: NotaryQueueRealtimeProviding {
    static let broadcastEvent = "request_changed"

    static func channelName(queueUserId: String) -> String {
        "notary-queue:\(queueUserId.trimmingCharacters(in: .whitespacesAndNewlines))"
    }
    ...
    func start(
        ...
        let channel = client.channel(Self.channelName(queueUserId: queueUserId)) { config in
